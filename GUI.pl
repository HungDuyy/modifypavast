...
# Creating GUI
####################################################################################################
my $main = MainWindow->new(-title => 'ModifyPaVaSt_Real32_GUI - Modified by Hung Duy');
#Adjust the size of the main window.
$main->geometry("500x200");
$main->resizable(0,0);
my $start_dir = ".";

my $selModuleArxml = $main -> Entry(-width=>30) -> grid(-row=>1,-column=>1);
my $ButRefModuleArxml = $main -> Button(-width=>20, -text => "Select Module ARXML", 
                           -font=>"ansi 8 bold",
		                   -command =>\&ButRefModuleArxml_push) -> grid(-row=>1,-column=>2);

my $selPaVaStFile = $main->Entry(-width=>30) -> grid(-row=>2,-column=>1);
my $ButPaVastFile = $main->Button(-width=>20, -text => "Select PaVaSt File", 
                        -font=>"ansi 8 bold",
						-command =>\&ButPaVaStFile_push) -> grid(-row=>2,-column=>2);
            
            
#Generate Options
my $frmOptions = $main->Frame();
$frmOptions->grid(-row=>3,-column=>1);
my $lblOptions = $frmOptions->Label(-text=>"RTE Buffer Naming Option ",-font=>"ansi 8 bold");
$lblOptions->grid(-row=>5,-column=>2);
my $option1 = $frmOptions->Radiobutton(-text=>"Rte_", -value=>"option1",  -variable=>\$option);
$option1->grid(-row=>7,-column=>2);
my $option2 = $frmOptions->Radiobutton(-text=>"_RTE(Default)", -value=>"option2",  -variable=>\$option);
$option2->grid(-row=>11,-column=>2);
my $option3 = $frmOptions->Radiobutton(-text=>"Same as Data Element", -value=>"option3",  -variable=>\$option);
$option3->grid(-row=>13,-column=>2);
		
my $btnGenerateFile = $main -> Button(-text => "Update PaVaSt File!",
                                      -state=>'disabled', 
									                    -font=>"ansi 8 bold",
		                                  -command =>\&ButRefStart_push) -> grid(-row=>15,-column=>1);
...
sub updateButtonState {
  ...
  if (($pavast ne "") and ($moduleArxml ne "") and ($buttonState eq 'disabled')) {
...
sub ButRefModuleArxml_push {
  $moduleArxml = $main->getOpenFile(-initialdir => $start_dir,                            
                              -title => 'Select Module ARXML file',
                              -filetypes =>[['ARXML Files',  '.arxml'],['All Files',  '*']]);							
  $selModuleArxml->delete(0,'end');
  $selModuleArxml->insert('end',$moduleArxml);
  if (!defined $moduleArxml) { $moduleArxml = ''; }	
}

sub ButPaVaStFile_push {
  $pavast = $main->getOpenFile(-initialdir => $start_dir,                            
                              -title => 'Select PaVast file to be updated',
                              -filetypes =>[['XML Files',  '.xml'],['All Files',  '*']]);							 
  $selPaVaStFile->delete(0,'end');
  $selPaVaStFile->insert('end',$pavast);
  if (!defined $pavast) { $pavast = ''; }	
}
...
sub ButRefStart_push
 {
    ...
    if(readModuleArxml($moduleArxml))
    {
      ...
      {
	      $main -> messageBox(-type=>"ok", -message=>"Updated PaVaSt file generated and stored in $new_pavast. Please review file content.");
	    }
	    else
      {
	      $main -> messageBox(-type=>"ok", -message=>"Error while reading PaVaSt file. Please check file content.");
	    }
    }
    else
    {
      $main -> messageBox(-type=>"ok", -message=>"Error while reading Module ARXML file. Please check file content.");
    }	
    exit;
}
...
{  
  ...
  if($ArRelease eq "4.0")
  {
    my($filename, $dirs, $suffix) = fileparse($moduleArxml);  
    $new_pavast = $dirs.$appSWCName."_pavast.xml";
  }
  else
  {
...
{  
  ...
  if($ArRelease eq "4.0")
  {
    my @adminDataSDGS = ($element->getElementsByTagName("ADMIN-DATA"))[0]->getElementsByTagName("SDGS");
    my @sd = $adminDataSDGS[0]->getElementsByTagName("SD");
    foreach my $currentSD(@sd)
    {
      if($currentSD->getAttribute("GID") eq "ASCET-Component-Name")
      {
        $currentSD->removeChildNodes();
        $currentSD->appendText($appSWCName);      
      }
    }
    
    my @component = ($element->getElementsByTagName("SW-COMPONENTS"))[0]->getChildrenByTagName("SW-FEATURE");
    my @componentName = $component[0]->getChildrenByTagName("SHORT-NAME");  
    replaceShortName($componentName[0],$appSWCName);    
    my $appSWCNameEx = $appSWCName."_Ex";
    my $appSWCNameIm = $appSWCName."_Im";
    my @componentEx = ($component[0]->getElementsByTagName("SW-FEATURE-INTERFACE"))[0]->getChildrenByTagName("SHORT-NAME"); 
    replaceShortName($componentEx[0],$appSWCNameEx); 
    my @componentIm = ($component[0]->getElementsByTagName("SW-FEATURE-INTERFACE"))[1]->getChildrenByTagName("SHORT-NAME"); 
    replaceShortName($componentIm[0],$appSWCNameIm);    
    my @swInstanceSpec = ($element->getElementsByTagName("SW-INSTANCE-SPEC"))[0]->getChildrenByTagName("SW-INSTANCE-TREE");
    my @swInstanceSpecName = $swInstanceSpec[0]->getChildrenByTagName("SHORT-NAME");  
    replaceShortName($swInstanceSpecName[0],$appSWCName);
    my @swCollectionSpec = ($element->getElementsByTagName("SW-COLLECTION-SPEC"))[0]->getElementsByTagName("SW-FEATURE-REFS");
    my @swCollectionSpecRef = $swCollectionSpec[0]->getChildrenByTagName("SW-FEATURE-REF");  
    replaceSwFeatureRef($swCollectionSpecRef[0],$appSWCName);
  }
######################################################################################################################################################################################################### 
